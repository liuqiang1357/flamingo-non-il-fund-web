...
const NEOLINE_WAS_CONNECTED_KEY = 'NEOLINE_WAS_CONNECTED';
...
class NeoLine extends BaseWallet {
  ...
  protected async checkInstalled(): Promise<boolean> {
    const installed = await new Promise<boolean>(resolve => {
      const checkReady = () => {
        if (window.NEOLine != undefined && window.NEOLineN3 != undefined) {
          resolve(true);
        }
      };
  ...
  protected wasConnected(): boolean {
    return sessionStorage.getItem(NEOLINE_WAS_CONNECTED_KEY) === 'true';
  }

  protected async connectInternal(): Promise<void> {
    try {
      await this.neoDapiN3.getAccount();
      sessionStorage.setItem(NEOLINE_WAS_CONNECTED_KEY, 'true');
    } catch (error) {
      throw this.convertWalletError(error);
    }
  }
  ...
  protected async disconnectInternal(): Promise<void> {
    sessionStorage.removeItem(NEOLINE_WAS_CONNECTED_KEY);
  }
